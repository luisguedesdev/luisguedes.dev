      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {skillCategories.map((category, index) => (
            <div
              key={index}
              className={`bg-marinho rounded-2xl p-8 border-2 ${category.color} hover:shadow-xl hover:scale-105 transition-all duration-300`}
            >
                  ...
                  <div key={skill} className="flex items-center">
                    <div
                      className={`w-2 h-2 rounded-full mr-3 ${
                        category.color.includes("ciano")
                          ? "bg-ciano"
                          : category.color.includes("turquesa")
                          ? "bg-turquesa"
                          : "bg-azul"
                      }`}
                    ></div>
              ...
              {["JavaScript", "React", "Node.js", "TypeScript"].map((skill) => (
                <div key={skill}>
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <div
                      className="bg-gradient-to-r from-ciano to-turquesa h-2 rounded-full"
                      style={{ width: "90%" }}
                    ></div>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              {["Python", "Docker", "AWS", "MongoDB"].map((skill) => (
                <div key={skill}>
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <div
                      className="bg-gradient-to-r from-turquesa to-azul h-2 rounded-full"
                      style={{ width: "75%" }}
                    ></div>
                  </div>
                </div>
