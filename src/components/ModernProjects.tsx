    ...
    <section id="projects" className="py-20 bg-marinho">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <span className="text-turquesa font-semibold text-lg uppercase tracking-wider">
            {t("projects.subtitle")}
          </span>
        ...
        {projects.length > 0 ? (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
              {projects.map((project) => (
                <div
                  key={project.id}
                  className="group bg-gray-900 rounded-2xl p-8 border border-gray-700 hover:border-ciano transition-all duration-300 hover:transform hover:scale-105"
                >
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {project.technologies.map((tech: string) => (
                      <span
                        key={tech}
                        className="px-3 py-1 bg-marinho border border-turquesa text-turquesa text-sm rounded-full"
                      >
                        {tech}
                      </span>
                    ))}
          ...
          <div className="text-center py-12">
            <p className="text-gray-400 text-lg">
              {language === "en"
                ? "Projects coming soon! Check back later or visit my GitHub."
                : "Projetos em breve! Volte mais tarde ou visite meu GitHub."}
            </p>
            <a
              href="https://github.com/luisguedesdev"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center mt-6 px-8 py-3 bg-transparent border-2 border-ciano text-ciano font-bold rounded-lg hover:bg-ciano hover:text-marinho transition-all duration-200"
            >
              GitHub
              <svg
                className="w-5 h-5 ml-2"
